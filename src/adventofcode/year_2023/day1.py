...
def handle_string(input: str) -> int:
    ...
    last_matches = [-1] * 10
    for digit, spelled_digit in enumerate(digits_dict):
        edited_string = edited_string.replace(str(digit), spelled_digit)
    for i in range(1, 10):
        f_match = edited_string.find(digits_dict[i])
        first_matches[i] = f_match if f_match != -1 else 100
        last_matches[i] = edited_string.rfind(digits_dict[i])
    return int(f"{first_matches.index(min(first_matches))}{last_matches.index(max(last_matches))}")
